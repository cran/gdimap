synthfiberss2z <-
function(g0, angles=c(20,100), b=3000, S0=1, sigma=NULL, logplot=TRUE, pos=c(0,0,0), showglyph=FALSE, new=TRUE)
{
  ...
  if(showglyph) {
    ...
    if(logplot) 
      svl <- log(sv) # apply to view orientation
    tc <- delaunayn(g0)
    pc <- g0*svl
    if(new)
      open3d()
